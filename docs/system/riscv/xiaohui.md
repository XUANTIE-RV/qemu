...
|--------|--------|---------|-------------|
| `linux` | `on` / `off` | `off` | **Boot mode**: `on` for Linux (OpenSBI + kernel), `off` for bare-metal / RTOS |
| `aia` | `none` / `aplic` / `aplic-imsic` | `none` | **Interrupt controller**: `none` = PLIC, `aplic` = APLIC direct, `aplic-imsic` = APLIC + IMSIC (MSI) |
| `aclint` | `clint` / `aclint` / `per-hart` | `clint` | **Timer/IPI controller**: `clint` = SiFive CLINT, `aclint` = RISC-V ACLINT, `per-hart` = per-hart ACLINT |
| `iopmp` | `on` / `off` | `off` | Enable IOPMP (IO Physical Memory Protection) for DMA transactions |
| `pcu` | `on` / `off` | `off` | Enable PCU (Power Control Unit) and CPR `use-pcu-rvba` for per-core power management |
| `clic-v0p10` | `on` / `off` | `off` | Use CLIC v0.10 specification (older revision) |
| `sync-monchipba` | `on` / `off` | `off` | Synchronize `xt_monchipba` CSR across all harts to APLIC M-mode base |
...
> **Note**: APLIC M/S use a different (more compact) layout in this mode
> than in `aia=aplic`. Each domain occupies the minimum legal aperture
> (`APLIC_MIN_SIZE` = 16 KiB) because wired interrupts are converted to
> MSIs and routed via IMSIC, so the APLIC itself does not need the full
> 32 KiB region used by the direct-delivery mode.
